...
def get_gradient_color(index, total):
    ...
    # Mapear índice a posición en gradiente
    if total > 1:
    ...
    if ingredientes_df is not None and not ingredientes_df.empty:
        ...
        if (ingredientes_df_filtrado is not None and not ingredientes_df_filtrado.empty and
            ...
            with st.expander("🔹 Ingredientes en la Fórmula", expanded=True):
                # Ordenar ingredientes por % descendente
                ...
                for ing, pct in ing_list_sorted:
                    if (ingredientes_df_filtrado is not None and
                            "Ingrediente" in ingredientes_df_filtrado.columns and
                            ing in ingredientes_df_filtrado["Ingrediente"].values):
                        precio = ingredientes_df_filtrado[
                            ingredientes_df_filtrado["Ingrediente"] == ing
                        ]["precio"].values[0]
                        costo_aportado = (precio * pct / 100) * 10  # costo aportado por ingrediente (por 1000 kg)
                        ing_costos[ing] = costo_aportado
                    else:
                        ing_costos[ing] = 0

                # Mostrar cada ingrediente con barra gradiente
            ...
            # ---- Resumen de Cumplimiento (debajo de ingredientes) ----
            st.markdown("---")
            ...
            with col4:
                st.metric("📊 Ingredientes", f"{total_ings}")
